			...
			.extend(
					Datepicker.prototype,
					{
						...
						_dialogDatepicker : function(input, date, onSelect,
							...
							this._dialogInput.val(date).change();
...
/*
 * Atlassian UI and the Atlassian Design Guidelines are created by Atlassian.
 * See https://developer.atlassian.com/display/AUI/ for API documentation and
 * https://developer.atlassian.com/design/ for license details.
 */
		...
		var g = {
			...
			log : function() {
				if (typeof console != "undefined" && console.log) {
					Function.prototype.apply.apply(console.log, [ console,
							arguments ])
				}
			},
			I18n : {
			...
			populateParameters : function() {
				...
				this.$(".parameters input").each(
						function() {
							var j = this.value, m = this.title || this.id;
							if (i.$(this).hasClass("list")) {
								...
								i.params[m] = (j.match(/^(tru|fals)e$/i) ? j
										.toLowerCase() == "true" : j)
							}
						})
			},
			...
			alphanum : function(s, r) {
				...
				for (var m = 0; m < q; m++) {
					...
					var t = parseInt(o[m], 10), p = parseInt(j[m], 10);
					if (t == o[m] && p == j[m] && t != p) {
			...
			dim : function(m) {
				if (!AJS.dim.$dim) {
					...
					if (AJS.$.browser.msie) {
						AJS.dim.$dim.css({
							width : "200%",
							height : Math.max(AJS.$(document).height(), AJS.$(
									window).height())
									+ "px"
						})
					}
								...
								.each(
										function() {
											if (this.nodeName.toLowerCase() === "iframe") {
												if (j.exec(this.src)[0] === i
														&& AJS
																.$(this)
																.contents()
																.find(
																		"object, embed").length === 0) {
													return true
												}
											}
											AJS.hasFlash = true;
					...
					if (AJS.$.browser.msie && (m !== false || AJS.hasFlash)) {
						AJS.dim.shim = AJS
								.$('<iframe frameBorder="0" class="aui-blanket-shim" src="javascript:false;"/>');
						AJS.dim.shim.css({
							height : Math.max(AJS.$(document).height(), AJS.$(
									window).height())
									+ "px"
					...
					if (AJS.$.browser.msie
							&& parseInt(AJS.$.browser.version, 10) < 8) {
			...
			undim : function() {
				if (AJS.dim.$dim) {
					...
					if (AJS.$.browser.safari) {
						...
						AJS.$(window).scrollTop(10 + 5 * (i == 10))
								.scrollTop(i)
					}
				}
			},
								...
								function() {
									if (i.size != i.width()) {
										...
										for (var m = 0, n = AJS.onTextResize["on-text-resize"].length; m < n; m++) {
											AJS.onTextResize["on-text-resize"][m]
													(i.size)
										}
									}
								}, 0);
						AJS.onTextResize.em = i;
						AJS.onTextResize["on-text-resize"] = [ function(m) {
							j(m)
						} ]
					}
				}
			...
			drawLogo : function(t) {
				...
				var m = t.scaleFactor || 1, r = t.fill || "#fff", s = t.stroke
						|| "#000", i = 400 * m, p = 40 * m, o = t.strokeWidth || 1, q = t.containerID
						|| ".aui-logo";
			...
			id : function(i) {
				...
				if (document.getElementById(d) === null) {
					...
					if (document.getElementById(d) === null) {
						...
						throw new Error("ERROR: timestamped fallback ID " + d
								+ " exists. AJS.id stopped.")
					}
				}
			},
			_addID : function(j, m) {
				var i = AJS.$(j), n = m || false;
				i.each(function(p) {
					var o = AJS.$(this);
					if (!o.attr("id")) {
						o.attr("id", AJS.id(n))
...
(function() {
	...
	AJS.contextPath = function() {
		var b = null;
		var d = [ a, window.contextPath,
				window.Confluence && Confluence.getContextPath(),
				window.BAMBOO && BAMBOO.contextPath,
				window.FECRU && FECRU.pageContext ];
...
(function() {
	var b = "AJS.conglomerate.cookie", f = /(\\|^"|"$)/g, g = /\|\|+/g, i = /"/g, c = /[.*+?|^$()[\]{\\]/g;
	function e(l, o) {
		o = o || "";
		var n = new RegExp(d(l) + "=([^|]+)"), m = o.match(n);
		return m && m[1]
	}
	function j(l, n, p) {
	...
	function k(l) {
		var n = new RegExp("\\b" + d(l) + "=((?:[^\\\\;]+|\\\\.)*)(?:;|$)"), m = document.cookie
				.match(n);
		return m && h(m[1])
	}
	function a(m, o, q) {
...
AJS.popup = function(i) {
	...
	b.hide();
	AJS.enable(b);
	var c = AJS.$(".aui-blanket"), a = function(j, k) {
		var l = AJS.$(j, k);
		if (l.length) {
			l.focus();
			return true
	...
	var f = {
		...
		show : function() {
			var j = function() {
				AJS.$(document).off("keydown", i.keypressListener).on(
						"keydown", i.keypressListener);
				AJS.dim();
				c = AJS.$(".aui-blanket");
		...
		disable : function() {
			if (!this.disabled) {
				this.popupBlanket = AJS
						.$("<div class='dialog-blanket'> </div>").css({
							height : b.height(),
							width : b.width()
						});
...
(function() {
	...
	var h = function(q, r, k, p, n) {
		...
		this.body = AJS("div").append(k).addClass("dialog-panel-body").css(
				"height", q.dialog.height + "px");
		this.padding = d;
		if (p) {
			this.body.addClass(p)
		}
		var m = q.panel.length, o = this;
	...
	var j = function(k, l) {
		...
		this.menu = AJS("ul").addClass("dialog-page-menu").css("height",
				k.height + "px");
		...
		k.popup.element
				.append(this.element.append(this.menu).append(this.body));
				...
				for (var A = x.length; A--;) {
					if (C || (v == "panel" && (E || (!E && w == null)))) {
						for (var z = x[A].panel.length; z--;) {
							if (x[A].panel[z].button.html() == C
									|| x[A].panel[z].button.html() == E
									|| (v == "panel" && !E && w == null)) {
								l.push(x[A].panel[z])
							}
						}
					...
					if (C || (v == "button" && (E || (!E && w == null)))) {
						for (var z = x[A].button.length; z--;) {
							if (x[A].button[z].item.html() == C
									|| x[A].button[z].item.html() == E
									|| (v == "button" && !E && w == null)) {
								l.push(x[A].button[z])
							}
						}
	...
	AJS.Dialog.prototype.isMaximised = function() {
		return this.popup.element.outerHeight() >= AJS.$(window).height()
				- (i * 2)
	};
...
(function(a) {
	...
	AJS.DatePicker = function(h, d) {
		...
		c = function() {
			...
			o = function() {
				...
				a.extend(t,
						AJS.DatePicker.prototype.localisations[d.languageCode]);
				if (d.firstDay > -1) {
					t.firstDay = d.firstDay
				}
				if (typeof e.attr("step") !== "undefined") {
					AJS
							.log("WARNING: The AJS date picker polyfill currently does not support the step attribute!")
				}
				l.datepicker(t);
				e.on("focusout", r);
			...
			if (d.overrideBrowserDefault
					&& AJS.DatePicker.prototype.browserSupportsDateField) {
				e[0].type = "text"
			}
		};
		g.reset = function() {
	...
	AJS.DatePicker.prototype.localisations = {
		...
		"ar-DZ" : {
			dayNames : [ "Ø§Ù„Ø£ØØ¯", "Ø§Ù„Ø§Ø«Ù†ÙŠÙ†", "Ø§Ù„Ø«Ù„Ø§Ø«Ø§Ø¡",
					"Ø§Ù„Ø£Ø±Ø¨Ø¹Ø§Ø¡", "Ø§Ù„Ø®Ù…ÙŠØ³", "Ø§Ù„Ø¬Ù…Ø¹Ø©",
					"Ø§Ù„Ø³Ø¨Øª" ],
			dayNamesMin : [ "Ø§Ù„Ø£ØØ¯", "Ø§Ù„Ø§Ø«Ù†ÙŠÙ†", "Ø§Ù„Ø«Ù„Ø§Ø«Ø§Ø¡",
					"Ø§Ù„Ø£Ø±Ø¨Ø¹Ø§Ø¡", "Ø§Ù„Ø®Ù…ÙŠØ³", "Ø§Ù„Ø¬Ù…Ø¹Ø©",
					"Ø§Ù„Ø³Ø¨Øª" ],
			firstDay : 6,
			isRTL : true,
			monthNames : [ "Ø¬Ø§Ù†ÙÙŠ", "ÙÙŠÙØ±ÙŠ", "Ù…Ø§Ø±Ø³",
					"Ø£ÙØ±ÙŠÙ„", "Ù…Ø§ÙŠ", "Ø¬ÙˆØ§Ù†", "Ø¬ÙˆÙŠÙ„ÙŠØ©",
					"Ø£ÙˆØª", "Ø³Ø¨ØªÙ…Ø¨Ø±", "Ø£ÙƒØªÙˆØ¨Ø±", "Ù†ÙˆÙÙ…Ø¨Ø±",
					"Ø¯ÙŠØ³Ù…Ø¨Ø±" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		ar : {
			...
			monthNames : [ "ÙƒØ§Ù†ÙˆÙ† Ø§Ù„Ø«Ø§Ù†ÙŠ", "Ø´Ø¨Ø§Ø·", "Ø¢Ø°Ø§Ø±",
					"Ù†ÙŠØ³Ø§Ù†", "Ù…Ø§ÙŠÙˆ", "ØØ²ÙŠØ±Ø§Ù†", "ØªÙ…ÙˆØ²",
		...
		az : {
			dayNames : [ "Bazar", "Bazar ertÉ™si", "Ã‡É™rÅŸÉ™nbÉ™ axÅŸamÄ±",
					"Ã‡É™rÅŸÉ™nbÉ™", "CÃ¼mÉ™ axÅŸamÄ±", "CÃ¼mÉ™", "ÅžÉ™nbÉ™" ],
			dayNamesMin : [ "B", "Be", "Ã‡a", "Ã‡", "Ca", "C", "Åž" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "Yanvar", "Fevral", "Mart", "Aprel", "May", "Ä°yun",
					"Ä°yul", "Avqust", "Sentyabr", "Oktyabr", "Noyabr",
					"Dekabr" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		bg : {
			dayNames : [ "ÐÐµÐ´ÐµÐ»Ñ", "ÐŸÐ¾Ð½ÐµÐ´ÐµÐ»Ð½Ð¸Ðº",
					"Ð’Ñ‚Ð¾Ñ€Ð½Ð¸Ðº", "Ð¡Ñ€ÑÐ´Ð°", "Ð§ÐµÑ‚Ð²ÑŠÑ€Ñ‚ÑŠÐº",
					"ÐŸÐµÑ‚ÑŠÐº", "Ð¡ÑŠÐ±Ð¾Ñ‚Ð°" ],
			dayNamesMin : [ "ÐÐµÐ´", "ÐŸÐ¾Ð½", "Ð’Ñ‚Ð¾", "Ð¡Ñ€Ñ", "Ð§ÐµÑ‚",
					"ÐŸÐµÑ‚", "Ð¡ÑŠÐ±" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "Ð¯Ð½ÑƒÐ°Ñ€Ð¸", "Ð¤ÐµÐ²Ñ€ÑƒÐ°Ñ€Ð¸", "ÐœÐ°Ñ€Ñ‚",
					"ÐÐ¿Ñ€Ð¸Ð»", "ÐœÐ°Ð¹", "Ð®Ð½Ð¸", "Ð®Ð»Ð¸", "ÐÐ²Ð³ÑƒÑÑ‚",
					"Ð¡ÐµÐ¿Ñ‚ÐµÐ¼Ð²Ñ€Ð¸", "ÐžÐºÑ‚Ð¾Ð¼Ð²Ñ€Ð¸", "ÐÐ¾ÐµÐ¼Ð²Ñ€Ð¸",
					"Ð”ÐµÐºÐµÐ¼Ð²Ñ€Ð¸" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		...
		cs : {
			dayNames : [ "nedÄ›le", "pondÄ›lÃ", "ÃºterÃ½", "stÅ™eda",
					"Ätvrtek", "pÃ¡tek", "sobota" ],
			dayNamesMin : [ "ne", "po", "Ãºt", "st", "Ät", "pÃ¡", "so" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "leden", "Ãºnor", "bÅ™ezen", "duben", "kvÄ›ten",
					"Äerven", "Äervenec", "srpen", "zÃ¡Å™Ã", "Å™Ãjen",
					"listopad", "prosinec" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		...
		da : {
			dayNames : [ "SÃ¸ndag", "Mandag", "Tirsdag", "Onsdag", "Torsdag",
					"Fredag", "LÃ¸rdag" ],
		...
		el : {
			dayNames : [ "ÎšÏ…ÏÎ¹Î±ÎºÎ®", "Î”ÎµÏ…Ï„ÎÏÎ±", "Î¤ÏÎ¯Ï„Î·",
					"Î¤ÎµÏ„Î¬ÏÏ„Î·", "Î ÎÎ¼Ï€Ï„Î·", "Î Î±ÏÎ±ÏƒÎºÎµÏ…Î®",
					"Î£Î¬Î²Î²Î±Ï„Î¿" ],
			dayNamesMin : [ "ÎšÏ…Ï", "Î”ÎµÏ…", "Î¤ÏÎ¹", "Î¤ÎµÏ„", "Î ÎµÎ¼",
					"Î Î±Ï", "Î£Î±Î²" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "Î™Î±Î½Î¿Ï…Î¬ÏÎ¹Î¿Ï‚", "Î¦ÎµÎ²ÏÎ¿Ï…Î¬ÏÎ¹Î¿Ï‚",
					"ÎœÎ¬ÏÏ„Î¹Î¿Ï‚", "Î‘Ï€ÏÎ¯Î»Î¹Î¿Ï‚", "ÎœÎ¬Î¹Î¿Ï‚",
					"Î™Î¿ÏÎ½Î¹Î¿Ï‚", "Î™Î¿ÏÎ»Î¹Î¿Ï‚", "Î‘ÏÎ³Î¿Ï…ÏƒÏ„Î¿Ï‚",
					"Î£ÎµÏ€Ï„ÎÎ¼Î²ÏÎ¹Î¿Ï‚", "ÎŸÎºÏ„ÏŽÎ²ÏÎ¹Î¿Ï‚",
					"ÎÎ¿ÎÎ¼Î²ÏÎ¹Î¿Ï‚", "Î”ÎµÎºÎÎ¼Î²ÏÎ¹Î¿Ï‚" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		"en-AU" : {
		...
		es : {
			...
			dayNamesMin : [ "Dom", "Lun", "Mar", "Mi&eacute;", "Juv", "Vie",
					"S&aacute;b" ],
			firstDay : 1,
			isRTL : false,
		...
		et : {
			dayNames : [ "PÃ¼hapÃ¤ev", "EsmaspÃ¤ev", "TeisipÃ¤ev",
					"KolmapÃ¤ev", "NeljapÃ¤ev", "Reede", "LaupÃ¤ev" ],
			dayNamesMin : [ "PÃ¼hap", "Esmasp", "Teisip", "Kolmap", "Neljap",
					"Reede", "Laup" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "Jaanuar", "Veebruar", "MÃ¤rts", "Aprill", "Mai",
		...
		fa : {
			dayNames : [ "ÙŠÚ©Ø´Ù†Ø¨Ù‡", "Ø¯ÙˆØ´Ù†Ø¨Ù‡", "Ø³Ù‡â€ŒØ´Ù†Ø¨Ù‡",
					"Ú†Ù‡Ø§Ø±Ø´Ù†Ø¨Ù‡", "Ù¾Ù†Ø¬Ø´Ù†Ø¨Ù‡", "Ø¬Ù…Ø¹Ù‡",
					"Ø´Ù†Ø¨Ù‡" ],
			dayNamesMin : [ "ÙŠ", "Ø¯", "Ø³", "Ú†", "Ù¾", "Ø¬", "Ø´" ],
			firstDay : 6,
			isRTL : true,
			monthNames : [ "ÙØ±ÙˆØ±Ø¯ÙŠÙ†", "Ø§Ø±Ø¯ÙŠØ¨Ù‡Ø´Øª", "Ø®Ø±Ø¯Ø§Ø¯",
					"ØªÙŠØ±", "Ù…Ø±Ø¯Ø§Ø¯", "Ø´Ù‡Ø±ÙŠÙˆØ±", "Ù…Ù‡Ø±",
					"Ø¢Ø¨Ø§Ù†", "Ø¢Ø°Ø±", "Ø¯ÙŠ", "Ø¨Ù‡Ù…Ù†", "Ø§Ø³ÙÙ†Ø¯" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		fi : {
		...
		fo : {
			dayNames : [ "Sunnudagur", "MÃ¡nadagur", "TÃ½sdagur", "Mikudagur",
					"HÃ³sdagur", "FrÃggjadagur", "Leyardagur" ],
			dayNamesMin : [ "Sun", "MÃ¡n", "TÃ½s", "Mik", "HÃ³s", "FrÃ", "Ley" ],
			firstDay : 0,
			isRTL : false,
			monthNames : [ "Januar", "Februar", "Mars", "AprÃl", "Mei", "Juni",
		...
		fr : {
			dayNames : [ "Dimanche", "Lundi", "Mardi", "Mercredi", "Jeudi",
					"Vendredi", "Samedi" ],
			dayNamesMin : [ "Dim.", "Lun.", "Mar.", "Mer.", "Jeu.", "Ven.",
					"Sam." ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "Janvier", "FÃ©vrier", "Mars", "Avril", "Mai",
		...
		gl : {
			...
			dayNamesMin : [ "Dom", "Lun", "Mar", "M&eacute;r", "Xov", "Ven",
					"S&aacute;b" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "Xaneiro", "Febreiro", "Marzo", "Abril", "Maio",
					"XuÃ±o", "Xullo", "Agosto", "Setembro", "Outubro",
					"Novembro", "Decembro" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		...
		hr : {
			...
			firstDay : 1,
			isRTL : false,
			monthNames : [ "SijeÄanj", "VeljaÄa", "OÅ¾ujak", "Travanj",
		...
		hu : {
			dayNames : [ "VasÃ¡rnap", "HÃ©tfÅ‘", "Kedd", "Szerda",
					"CsÃ¼tÃ¶rtÃ¶k", "PÃ©ntek", "Szombat" ],
			dayNamesMin : [ "Vas", "HÃ©t", "Ked", "Sze", "CsÃ¼", "PÃ©n", "Szo" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "JanuÃ¡r", "FebruÃ¡r", "MÃ¡rcius", "Ãprilis",
					"MÃ¡jus", "JÃºnius", "JÃºlius", "Augusztus", "Szeptember",
					"OktÃ³ber", "November", "December" ],
			showMonthAfterYear : true,
			yearSuffix : ""
		},
		hy : {
			dayNames : [ "Õ¯Õ«Ö€Õ¡Õ¯Õ«", "Õ¥Õ¯Õ¸Ö‚Õ·Õ¡Õ¢Õ©Õ«",
					"Õ¥Ö€Õ¥Ö„Õ·Õ¡Õ¢Õ©Õ«", "Õ¹Õ¸Ö€Õ¥Ö„Õ·Õ¡Õ¢Õ©Õ«",
					"Õ°Õ«Õ¶Õ£Õ·Õ¡Õ¢Õ©Õ«", "Õ¸Ö‚Ö€Õ¢Õ¡Õ©", "Õ·Õ¡Õ¢Õ¡Õ©" ],
			dayNamesMin : [ "Õ¯Õ«Ö€", "Õ¥Ö€Õ¯", "Õ¥Ö€Ö„", "Õ¹Ö€Ö„", "Õ°Õ¶Õ£",
					"Õ¸Ö‚Ö€Õ¢", "Õ·Õ¢Õ©" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "Õ€Õ¸Ö‚Õ¶Õ¾Õ¡Ö€", "Õ“Õ¥Õ¿Ö€Õ¾Õ¡Ö€", "Õ„Õ¡Ö€Õ¿",
					"Ô±ÕºÖ€Õ«Õ¬", "Õ„Õ¡ÕµÕ«Õ½", "Õ€Õ¸Ö‚Õ¶Õ«Õ½", "Õ€Õ¸Ö‚Õ¬Õ«Õ½",
					"Õ•Õ£Õ¸Õ½Õ¿Õ¸Õ½", "ÕÕ¥ÕºÕ¿Õ¥Õ´Õ¢Õ¥Ö€",
					"Õ€Õ¸Õ¯Õ¿Õ¥Õ´Õ¢Õ¥Ö€", "Õ†Õ¸ÕµÕ¥Õ´Õ¢Õ¥Ö€",
					"Ô´Õ¥Õ¯Õ¿Õ¥Õ´Õ¢Õ¥Ö€" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		...
		is : {
			dayNames : [ "Sunnudagur", "M&aacute;nudagur",
					"&THORN;ri&eth;judagur", "Mi&eth;vikudagur", "Fimmtudagur",
					"F&ouml;studagur", "Laugardagur" ],
			dayNamesMin : [ "Sun", "M&aacute;n", "&THORN;ri", "Mi&eth;", "Fim",
					"F&ouml;s", "Lau" ],
			firstDay : 0,
			isRTL : false,
		...
		ja : {
			dayNames : [ "æ—¥æ›œæ—¥", "æœˆæ›œæ—¥", "ç«æ›œæ—¥", "æ°´æ›œæ—¥",
		...
		kk : {
			dayNames : [ "Ð–ÐµÐºÑÐµÐ½Ð±Ñ–", "Ð”Ò¯Ð¹ÑÐµÐ½Ð±Ñ–",
					"Ð¡ÐµÐ¹ÑÐµÐ½Ð±Ñ–", "Ð¡Ó™Ñ€ÑÐµÐ½Ð±Ñ–", "Ð‘ÐµÐ¹ÑÐµÐ½Ð±Ñ–",
					"Ð–Ò±Ð¼Ð°", "Ð¡ÐµÐ½Ð±Ñ–" ],
			dayNamesMin : [ "Ð¶ÐºÑ", "Ð´ÑÐ½", "ÑÑÐ½", "ÑÑ€Ñ", "Ð±ÑÐ½",
					"Ð¶Ð¼Ð°", "ÑÐ½Ð±" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "ÒšÐ°Ò£Ñ‚Ð°Ñ€", "ÐÒ›Ð¿Ð°Ð½", "ÐÐ°ÑƒÑ€Ñ‹Ð·",
					"Ð¡Ó™ÑƒÑ–Ñ€", "ÐœÐ°Ð¼Ñ‹Ñ€", "ÐœÐ°ÑƒÑÑ‹Ð¼", "Ð¨Ñ–Ð»Ð´Ðµ",
					"Ð¢Ð°Ð¼Ñ‹Ð·", "ÒšÑ‹Ñ€ÐºÒ¯Ð¹ÐµÐº", "ÒšÐ°Ð·Ð°Ð½",
					"ÒšÐ°Ñ€Ð°ÑˆÐ°", "Ð–ÐµÐ»Ñ‚Ð¾Ò›ÑÐ°Ð½" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		ko : {
			...
			firstDay : 0,
			isRTL : false,
			monthNames : [ "1ì›”(JAN)", "2ì›”(FEB)", "3ì›”(MAR)", "4ì›”(APR)",
					"5ì›”(MAY)", "6ì›”(JUN)", "7ì›”(JUL)", "8ì›”(AUG)",
					"9ì›”(SEP)", "10ì›”(OCT)", "11ì›”(NOV)", "12ì›”(DEC)" ],
			showMonthAfterYear : false,
			yearSuffix : "ë…„"
		},
		lb : {
			dayNames : [ "Sonndeg", "MÃ©indeg", "DÃ«nschdeg", "MÃ«ttwoch",
					"Donneschdeg", "Freideg", "Samschdeg" ],
			dayNamesMin : [ "Son", "MÃ©i", "DÃ«n", "MÃ«t", "Don", "Fre", "Sam" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "Januar", "Februar", "MÃ¤erz", "AbrÃ«ll", "Mee",
					"Juni", "Juli", "August", "September", "Oktober",
					"November", "Dezember" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		...
		lv : {
			dayNames : [ "svÄ“tdiena", "pirmdiena", "otrdiena", "treÅ¡diena",
					"ceturtdiena", "piektdiena", "sestdiena" ],
			dayNamesMin : [ "svt", "prm", "otr", "tre", "ctr", "pkt", "sst" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "JanvÄris", "FebruÄris", "Marts", "AprÄ«lis",
					"Maijs", "JÅ«nijs", "JÅ«lijs", "Augusts", "Septembris",
					"Oktobris", "Novembris", "Decembris" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		mk : {
			dayNames : [ "ÐÐµÐ´ÐµÐ»Ð°", "ÐŸÐ¾Ð½ÐµÐ´ÐµÐ»Ð½Ð¸Ðº",
					"Ð’Ñ‚Ð¾Ñ€Ð½Ð¸Ðº", "Ð¡Ñ€ÐµÐ´Ð°", "Ð§ÐµÑ‚Ð²Ñ€Ñ‚Ð¾Ðº",
					"ÐŸÐµÑ‚Ð¾Ðº", "Ð¡Ð°Ð±Ð¾Ñ‚Ð°" ],
			dayNamesMin : [ "ÐÐµÐ´", "ÐŸÐ¾Ð½", "Ð’Ñ‚Ð¾", "Ð¡Ñ€Ðµ", "Ð§ÐµÑ‚",
					"ÐŸÐµÑ‚", "Ð¡Ð°Ð±" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "ÐˆÐ°Ð½ÑƒÐ°Ñ€Ð¸", "Ð¤ÐµÐ±Ñ€ÑƒÐ°Ñ€Ð¸", "ÐœÐ°Ñ€Ñ‚",
					"ÐÐ¿Ñ€Ð¸Ð»", "ÐœÐ°Ñ˜", "ÐˆÑƒÐ½Ð¸", "ÐˆÑƒÐ»Ð¸",
					"ÐÐ²Ð³ÑƒÑÑ‚", "Ð¡ÐµÐ¿Ñ‚ÐµÐ¼Ð²Ñ€Ð¸", "ÐžÐºÑ‚Ð¾Ð¼Ð²Ñ€Ð¸",
					"ÐÐ¾ÐµÐ¼Ð²Ñ€Ð¸", "Ð”ÐµÐºÐµÐ¼Ð²Ñ€Ð¸" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		ml : {
			dayNames : [ "à´žà´¾à´¯à´°àµâ€", "à´¤à´¿à´™àµà´•à´³àµâ€",
					"à´šàµŠà´µàµà´µ", "à´¬àµà´§à´¨àµâ€",
					"à´µàµà´¯à´¾à´´à´‚", "à´µàµ†à´³àµà´³à´¿", "à´¶à´¨à´¿" ],
			dayNamesMin : [ "à´žà´¾à´¯", "à´¤à´¿à´™àµà´•", "à´šàµŠà´µàµà´µ",
					"à´¬àµà´§", "à´µàµà´¯à´¾à´´à´‚", "à´µàµ†à´³àµà´³à´¿",
					"à´¶à´¨à´¿" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "à´œà´¨àµà´µà´°à´¿", "à´«àµ†à´¬àµà´°àµà´µà´°à´¿",
					"à´®à´¾à´°àµâ€à´šàµà´šàµ", "à´à´ªàµà´°à´¿à´²àµâ€",
					"à´®àµ‡à´¯àµ", "à´œàµ‚à´£àµâ€", "à´œàµ‚à´²àµˆ",
					"à´†à´—à´¸àµà´±àµà´±àµ",
					"à´¸àµ†à´ªàµà´±àµà´±à´‚à´¬à´°àµâ€",
					"à´’à´•àµà´Ÿàµ‹à´¬à´°àµâ€", "à´¨à´µà´‚à´¬à´°àµâ€",
					"à´¡à´¿à´¸à´‚à´¬à´°àµâ€" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		...
		ro : {
			dayNames : [ "DuminicÄƒ", "Luni", "MarÅ£i", "Miercuri", "Joi",
					"Vineri", "SÃ¢mbÄƒtÄƒ" ],
		...
		ru : {
			...
			dayNamesMin : [ "Ð²ÑÐº", "Ð¿Ð½Ð´", "Ð²Ñ‚Ñ€", "ÑÑ€Ð´", "Ñ‡Ñ‚Ð²",
					"Ð¿Ñ‚Ð½", "ÑÐ±Ñ‚" ],
			firstDay : 1,
			isRTL : false,
		...
		sk : {
			dayNames : [ "NedeÄ¾a", "Pondelok", "Utorok", "Streda", "Å tvrtok",
					"Piatok", "Sobota" ],
			dayNamesMin : [ "Ned", "Pon", "Uto", "Str", "Å tv", "Pia", "Sob" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "JanuÃ¡r", "FebruÃ¡r", "Marec", "AprÃl", "MÃ¡j",
					"JÃºn", "JÃºl", "August", "September", "OktÃ³ber",
					"November", "December" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		sl : {
			...
			dayNamesMin : [ "Ned", "Pon", "Tor", "Sre", "&#x10C;et", "Pet",
					"Sob" ],
			firstDay : 1,
			isRTL : false,
		...
		sq : {
			dayNames : [ "E Diel", "E HÃ«nÃ«", "E MartÃ«", "E MÃ«rkurÃ«",
		...
		sr : {
			dayNames : [ "ÐÐµÐ´ÐµÑ™Ð°", "ÐŸÐ¾Ð½ÐµÐ´ÐµÑ™Ð°Ðº", "Ð£Ñ‚Ð¾Ñ€Ð°Ðº",
					"Ð¡Ñ€ÐµÐ´Ð°", "Ð§ÐµÑ‚Ð²Ñ€Ñ‚Ð°Ðº", "ÐŸÐµÑ‚Ð°Ðº",
					"Ð¡ÑƒÐ±Ð¾Ñ‚Ð°" ],
			dayNamesMin : [ "ÐÐµÐ´", "ÐŸÐ¾Ð½", "Ð£Ñ‚Ð¾", "Ð¡Ñ€Ðµ", "Ð§ÐµÑ‚",
					"ÐŸÐµÑ‚", "Ð¡ÑƒÐ±" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "ÐˆÐ°Ð½ÑƒÐ°Ñ€", "Ð¤ÐµÐ±Ñ€ÑƒÐ°Ñ€", "ÐœÐ°Ñ€Ñ‚",
					"ÐÐ¿Ñ€Ð¸Ð»", "ÐœÐ°Ñ˜", "ÐˆÑƒÐ½", "ÐˆÑƒÐ»", "ÐÐ²Ð³ÑƒÑÑ‚",
					"Ð¡ÐµÐ¿Ñ‚ÐµÐ¼Ð±Ð°Ñ€", "ÐžÐºÑ‚Ð¾Ð±Ð°Ñ€", "ÐÐ¾Ð²ÐµÐ¼Ð±Ð°Ñ€",
					"Ð”ÐµÑ†ÐµÐ¼Ð±Ð°Ñ€" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		...
		ta : {
			dayNames : [ "à®žà®¾à®¯à®¿à®±à¯à®±à¯à®•à¯à®•à®¿à®´à®®à¯ˆ",
			...
			dayNamesMin : [ "à®žà®¾à®¯à®¿à®±à¯", "à®¤à®¿à®™à¯à®•à®³à¯",
					"à®šà¯†à®µà¯à®µà®¾à®¯à¯", "à®ªà¯à®¤à®©à¯",
					"à®µà®¿à®¯à®¾à®´à®©à¯", "à®µà¯†à®³à¯à®³à®¿", "à®šà®©à®¿" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "à®¤à¯ˆ", "à®®à®¾à®šà®¿", "à®ªà®™à¯à®•à¯à®©à®¿",
					"à®šà®¿à®¤à¯à®¤à®¿à®°à¯ˆ", "à®µà¯ˆà®•à®¾à®šà®¿",
					"à®†à®©à®¿", "à®†à®Ÿà®¿", "à®†à®µà®£à®¿",
					"à®ªà¯à®°à®Ÿà¯à®Ÿà®¾à®šà®¿", "à®à®ªà¯à®ªà®šà®¿",
					"à®•à®¾à®°à¯à®¤à¯à®¤à®¿à®•à¯ˆ", "à®®à®¾à®°à¯à®•à®´à®¿" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		th : {
			dayNames : [ "à¸à¸²à¸—à¸´à¸•à¸¢à¹Œ", "à¸ˆà¸±à¸™à¸—à¸£à¹Œ",
					"à¸à¸±à¸‡à¸„à¸²à¸£", "à¸žà¸¸à¸˜",
					"à¸žà¸¤à¸«à¸±à¸ªà¸šà¸”à¸µ", "à¸¨à¸¸à¸à¸£à¹Œ",
					"à¹€à¸ªà¸²à¸£à¹Œ" ],
			dayNamesMin : [ "à¸à¸².", "à¸ˆ.", "à¸.", "à¸ž.", "à¸žà¸¤.", "à¸¨.",
					"à¸ª." ],
			firstDay : 0,
			isRTL : false,
			monthNames : [ "à¸¡à¸à¸£à¸²à¸„à¸¡",
					"à¸à¸¸à¸¡à¸ à¸²à¸žà¸±à¸™à¸˜à¹Œ", "à¸¡à¸µà¸™à¸²à¸„à¸¡",
					"à¹€à¸¡à¸©à¸²à¸¢à¸™", "à¸žà¸¤à¸©à¸ à¸²à¸„à¸¡",
					"à¸¡à¸´à¸–à¸¸à¸™à¸²à¸¢à¸™", "à¸à¸£à¸à¸Žà¸²à¸„à¸¡",
					"à¸ªà¸´à¸‡à¸«à¸²à¸„à¸¡", "à¸à¸±à¸™à¸¢à¸²à¸¢à¸™",
					"à¸•à¸¸à¸¥à¸²à¸„à¸¡", "à¸žà¸¤à¸¨à¸ˆà¸´à¸à¸²à¸¢à¸™",
					"à¸˜à¸±à¸™à¸§à¸²à¸„à¸¡" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		tj : {
			dayNames : [ "ÑÐºÑˆÐ°Ð½Ð±Ðµ", "Ð´ÑƒÑˆÐ°Ð½Ð±Ðµ", "ÑÐµÑˆÐ°Ð½Ð±Ðµ",
					"Ñ‡Ð¾Ñ€ÑˆÐ°Ð½Ð±Ðµ", "Ð¿Ð°Ð½Ò·ÑˆÐ°Ð½Ð±Ðµ", "Ò·ÑƒÐ¼ÑŠÐ°",
					"ÑˆÐ°Ð½Ð±Ðµ" ],
			dayNamesMin : [ "ÑÐºÑˆ", "Ð´ÑƒÑˆ", "ÑÐµÑˆ", "Ñ‡Ð¾Ñ€", "Ð¿Ð°Ð½",
					"Ò·ÑƒÐ¼", "ÑˆÐ°Ð½" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "Ð¯Ð½Ð²Ð°Ñ€", "Ð¤ÐµÐ²Ñ€Ð°Ð»", "ÐœÐ°Ñ€Ñ‚",
					"ÐÐ¿Ñ€ÐµÐ»", "ÐœÐ°Ð¹", "Ð˜ÑŽÐ½", "Ð˜ÑŽÐ»", "ÐÐ²Ð³ÑƒÑÑ‚",
					"Ð¡ÐµÐ½Ñ‚ÑÐ±Ñ€", "ÐžÐºÑ‚ÑÐ±Ñ€", "ÐÐ¾ÑÐ±Ñ€",
					"Ð”ÐµÐºÐ°Ð±Ñ€" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		tr : {
			dayNames : [ "Pazar", "Pazartesi", "SalÄ±", "Ã‡arÅŸamba",
					"PerÅŸembe", "Cuma", "Cumartesi" ],
			dayNamesMin : [ "Pz", "Pt", "Sa", "Ã‡a", "Pe", "Cu", "Ct" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "Ocak", "Åžubat", "Mart", "Nisan", "MayÄ±s",
		...
		uk : {
			dayNames : [ "Ð½ÐµÐ´Ñ–Ð»Ñ", "Ð¿Ð¾Ð½ÐµÐ´Ñ–Ð»Ð¾Ðº",
					"Ð²Ñ–Ð²Ñ‚Ð¾Ñ€Ð¾Ðº", "ÑÐµÑ€ÐµÐ´Ð°", "Ñ‡ÐµÑ‚Ð²ÐµÑ€",
					"Ð¿â€™ÑÑ‚Ð½Ð¸Ñ†Ñ", "ÑÑƒÐ±Ð¾Ñ‚Ð°" ],
			dayNamesMin : [ "Ð½ÐµÐ´", "Ð¿Ð½Ð´", "Ð²Ñ–Ð²", "ÑÑ€Ð´", "Ñ‡Ñ‚Ð²",
					"Ð¿Ñ‚Ð½", "ÑÐ±Ñ‚" ],
			firstDay : 1,
			isRTL : false,
			monthNames : [ "Ð¡Ñ–Ñ‡ÐµÐ½ÑŒ", "Ð›ÑŽÑ‚Ð¸Ð¹", "Ð‘ÐµÑ€ÐµÐ·ÐµÐ½ÑŒ",
					"ÐšÐ²Ñ–Ñ‚ÐµÐ½ÑŒ", "Ð¢Ñ€Ð°Ð²ÐµÐ½ÑŒ", "Ð§ÐµÑ€Ð²ÐµÐ½ÑŒ",
					"Ð›Ð¸Ð¿ÐµÐ½ÑŒ", "Ð¡ÐµÑ€Ð¿ÐµÐ½ÑŒ", "Ð’ÐµÑ€ÐµÑÐµÐ½ÑŒ",
					"Ð–Ð¾Ð²Ñ‚ÐµÐ½ÑŒ", "Ð›Ð¸ÑÑ‚Ð¾Ð¿Ð°Ð´", "Ð“Ñ€ÑƒÐ´ÐµÐ½ÑŒ" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		vi : {
			dayNames : [ "Chá»§ Nháºt", "Thá»© Hai", "Thá»© Ba", "Thá»© TÆ°",
					"Thá»© NÄƒm", "Thá»© SÃ¡u", "Thá»© Báº£y" ],
			dayNamesMin : [ "CN", "T2", "T3", "T4", "T5", "T6", "T7" ],
			firstDay : 0,
			isRTL : false,
			monthNames : [ "ThÃ¡ng Má»™t", "ThÃ¡ng Hai", "ThÃ¡ng Ba",
					"ThÃ¡ng TÆ°", "ThÃ¡ng NÄƒm", "ThÃ¡ng SÃ¡u", "ThÃ¡ng Báº£y",
					"ThÃ¡ng TÃ¡m", "ThÃ¡ng ChÃn", "ThÃ¡ng MÆ°á»i",
					"ThÃ¡ng MÆ°á»i Má»™t", "ThÃ¡ng MÆ°á»i Hai" ],
			showMonthAfterYear : false,
			yearSuffix : ""
		},
		"zh-CN" : {
		...
		"zh-HK" : {
			dayNames : [ "æ˜ŸæœŸæ—¥", "æ˜ŸæœŸä¸€", "æ˜ŸæœŸäºŒ", "æ˜ŸæœŸä¸‰",
					"æ˜ŸæœŸå››", "æ˜ŸæœŸäº”", "æ˜ŸæœŸå…" ],
			dayNamesMin : [ "å‘¨æ—¥", "å‘¨ä¸€", "å‘¨äºŒ", "å‘¨ä¸‰", "å‘¨å››",
					"å‘¨äº”", "å‘¨å…" ],
			firstDay : 0,
			isRTL : false,
			monthNames : [ "ä¸€æœˆ", "äºŒæœˆ", "ä¸‰æœˆ", "å››æœˆ", "äº”æœˆ",
					"å…æœˆ", "ä¸ƒæœˆ", "å…«æœˆ", "ä¹æœˆ", "åæœˆ",
					"åä¸€æœˆ", "åäºŒæœˆ" ],
			showMonthAfterYear : true,
			yearSuffix : "å¹´"
		},
...
AJS.dropDown = function(n, e) {
	var w = null, k = [], s = false, h = AJS.$(document), c = {
		item : "li:has(a)",
		activeClass : "active",
		alignment : "right",
	...
	if (n && n.jquery) {
		...
		if (typeof n == "string") {
			...
			if (n && n.constructor == Array) {
				w = AJS("div").addClass("aui-dropdown").toggleClass("hidden",
						!!c.isHiddenByDefault);
				for (var r = 0, m = n.length; r < m; r++) {
					...
					for (var q = 0, u = n[r].length; q < u; q++) {
						...
						if (g.insideSpanIcon) {
							o.children("a").prepend(
									AJS("span").attr("class", "icon"))
						}
				...
				throw new Error(
						"AJS.dropDown function was called with illegal parameter. Should be AJS.$ object, AJS.$ selector or array.")
			}
		}
	}
	var f = function() {
	...
	var p = function(z) {
		var y = !s, i = AJS.dropDown.current.$[0], j = AJS.dropDown.current.links, A = i.focused;
		s = true;
		if (j.length === 0) {
			return
		}
	...
	var x = function(y) {
		...
		var z = y.which, i = AJS.dropDown.current.$[0], j = AJS.dropDown.current.links;
		...
		case 13:
			if (i.focused >= 0) {
				if (!c.selectionHandler) {
					...
					return c.selectionHandler.call(AJS.dropDown.current, y, AJS
							.$(j[i.focused]))
				}
			}
			return true;
	...
	var a = function(i) {
		if (!((i && i.which && (i.which == 3))
				|| (i && i.button && (i.button == 2)) || false)) {
			if (AJS.dropDown.current) {
	...
	w
			.each(function() {
								...
								.extend(
										z,
										{
											...
											getFocusIndex : function() {
												return (typeof i.focused == "number") ? i.focused
														: -1
											}
										});
				...
				z.show = function(A) {
					if (c.useDisabled
							&& this.$.closest(".aui-dd-parent").hasClass(
									"disabled")) {
						return
					}
					...
					j[this.method](true);
					AJS.$(document).trigger("showLayer",
							[ "dropdown", AJS.dropDown.current ])
				};
				z.hide = function(A) {
					...
					AJS.$(document).trigger("hideLayer",
							[ "dropdown", AJS.dropDown.current ]);
				...
				AJS.dropDown.createShims = function() {
					AJS.$("iframe").each(
							function(A) {
								var B = this;
								if (!B.shim) {
									B.shim = AJS.$("<div />").addClass(
											"shim hidden").appendTo("body");
						...
						var A = function() {
							if (this.$.is(":visible")) {
								if (!this.iframeShim) {
									this.iframeShim = AJS
											.$(
													'<iframe class="dropdown-shim" src="javascript:false;" frameBorder="0" />')
											.insertBefore(this.$)
								}
								this.iframeShim.css({
									display : "block",
...
AJS.dropDown.getAdditionalPropertyValue = function(d, a) {
	...
	if (!c || (typeof c.tagName != "string") || c.tagName.toLowerCase() != "li") {
		AJS
				.log("AJS.dropDown.getAdditionalPropertyValue : item passed in should be an LI element wrapped by jQuery")
	}
...
AJS.dropDown.Standard = function(h) {
	...
	if (b.useLiveEvents) {
		...
		g = g.not(".dd-allocated").filter(":has(" + b.dropDown + ")").filter(
				":has(" + b.trigger + ")");
		g.each(function() {
			var l = AJS.$(this), k = AJS.$(b.dropDown, this), i = AJS.$(
					b.trigger, this), j = AJS.dropDown(k, b)[0];
			AJS.$.extend(j, {
				trigger : i
			});
			f(i, l, k, j);
...
(function(g) {
	var e = g(document), j = (AJS.$.browser.msie && parseInt(
			AJS.$.browser.version, 10) == 8);
	var c = null;
	var b = (function() {
		var n = false;
		function l(q) {
		...
		"aui-button-invoke" : function(H, r) {
			...
			if (D) {
				...
				if (z) {
					...
					throw new Error(
							"The specified data-dropdown2-hide-location id doesn't exist")
				}
			}
			...
			o.find("li.hidden > a").addClass("disabled").attr("aria-disabled",
					"true");
			...
			function q() {
				...
				var ad = "left";
				if (j) {
					var V = parseInt(o.css("border-left-width"), 10)
				...
				var W = ab.left, ac = ab.top + n.outerHeight();
				if (R) {
					var Y = 3;
				...
				if (U) {
					var S = n.closest(U), Z = n.offset().left + n.outerWidth(), X = Z
							+ ae;
					if (T >= ae) {
						ae = T
					}
			...
			function A() {
				...
				I("off");
				setTimeout(function() {
					o.css("display", "none").css("min-width", "")
							.insertAfter(n).attr("aria-hidden", "true");
					if (!R) {
			...
			function J(T) {
				var S = o
						.find(
								"> ul > li > a, > .aui-dropdown2-section > ul > li > a")
						.not(".disabled");
				u(K(S, T, true))
			}
			function B(S) {
			...
			function Q(S) {
				B(K(
						t
								.find(".aui-dropdown2-trigger")
								.not(
										".disabled, [aria-disabled=true], .aui-dropdown2-sub-trigger"),
						S, false))
			}
			function K(U, V, T) {
			...
			function I(T) {
				...
				if (!R) {
					...
					E[U]
							(
									"aui-dropdown2-hide aui-dropdown2-item-selected aui-dropdown2-step-out",
									G)
				}
	...
	var h = {
		mouseup : function(l) {
			var m = g(l.target).closest(
					".aui-dropdown2 a, .aui-dropdown2-trigger")[0];
			if (m) {
				setTimeout(function() {
					k(m)
				}, 0)
	...
	function k(l) {
		if (l.click) {
			...
			m.initMouseEvent("click", true, true, window, 0, 0, 0, 0, 0, false,
	...
	function i(n) {
		...
		var o = n.attr("aria-owns"), l = n.attr("aria-haspopup"), m = document
				.getElementById(o);
		if (m) {
			return g(m)
		} else {
			if (!o) {
				throw new Error(
						"Dropdown 2 trigger required attribute not set: aria-owns")
			}
			if (!l) {
				throw new Error(
						"Dropdown 2 trigger required attribute not set: aria-haspopup")
			}
			if (!m) {
				throw new Error(
						"Dropdown 2 trigger aria-owns attr set to nonexistent id: "
								+ o)
			}
			throw new Error(
					"Dropdown 2 trigger unknown error. I don't know what you did, but there's smoke everywhere. Consult the documentation.")
		}
	}
	e.delegate(".aui-dropdown2-trigger", b);
	e
			.delegate(
					".aui-dropdown2-trigger:not(.active):not([aria-disabled=true]),.aui-dropdown2-sub-trigger:not([aria-disabled=true])",
					a);
	...
	e.delegate(".aui-dropdown2-radio:not(.checked):not(.disabled)", "click",
			function() {
				var l = g(this);
				var m = l.closest("ul").find(".checked");
				m.removeClass("checked").attr("aria-checked", "false").trigger(
						"aui-dropdown2-item-uncheck");
				l.addClass("checked").attr("aria-checked", "true").trigger(
						"aui-dropdown2-item-check")
			});
...
(function(a) {
	AJS.InlineDialog = function(t, h, k, i) {
		if (i && i.getArrowAttributes) {
			AJS
					.log("DEPRECATED: getArrowAttributes - See https://ecosystem.atlassian.net/browse/AUI-1362")
		}
		if (i && i.getArrowPath) {
			AJS
					.log("DEPRECATED: getArrowPath - See https://ecosystem.atlassian.net/browse/AUI-1362")
		}
		if (typeof h === "undefined") {
			h = String(Math.random()).replace(".", "");
			if (a("#inline-dialog-" + h + ", #arrow-" + h
					+ ", #inline-dialog-shim-" + h).length) {
		...
		var A;
		var p;
		var b = a('<div id="inline-dialog-'
				+ h
				+ '" class="aui-inline-dialog"><div class="contents"></div><div id="arrow-'
				+ h + '" class="arrow"></div></div>');
		var g = a("#arrow-" + h, b);
				...
				e = {
					...
					reset : function() {
						function E(G, F) {
							G.css(F.popupCss);
							if (window.Raphael && i
									&& (i.getArrowPath || i.getArrowAttributes)) {
								if (F.displayAbove) {
									F.arrowCss.top -= AJS.$.browser.msie ? 10
											: 9
								}
								if (!G.arrowCanvas) {
									G.arrowCanvas = Raphael("arrow-" + h, 16,
											16)
								}
								var H = r.getArrowPath, I = a.isFunction(H) ? H(F)
										: H;
								G.arrowCanvas.path(I).attr(
										r.getArrowAttributes())
							} else {
								if (!g.hasClass("aui-css-arrow")) {
									g.addClass("aui-css-arrow")
								}
								if (F.displayAbove
										&& !g.hasClass("aui-bottom-arrow")) {
						...
						if (AJS.$.browser.msie
								&& ~~(AJS.$.browser.version) < 10) {
							var D = a("#inline-dialog-shim-" + h);
							if (D.length == 0) {
								a(b)
										.prepend(
												a('<iframe class = "inline-dialog-shim" id="inline-dialog-shim-'
														+ h
...
(function() {
	...
	AJS.messages = {
		...
		makeCloseable : function(d) {
			AJS
					.$(d || "div.aui-message.closeable")
					.each(
							function() {
								var f = AJS.$(this), e = AJS
										.$(
										...
										.keypress(
												function(g) {
													if ((g.which === AJS.keyCode.ENTER)
															|| (g.which === AJS.keyCode.SPACE)) {
														f.closeMessage();
														g.preventDefault()
													}
												});
		...
		createMessage : function(d) {
			AJS.messages[d] = function(g, i) {
				...
				if (i.id) {
					if (/[#\'\"\.\s]/g.test(i.id)) {
						AJS
								.log("AJS.Messages error: ID rejected, must not include spaces, hashes, dots or quotes.")
					} else {
						f.attr("id", i.id)
					}
				}
...
(function() {
	var c = /#.*/, d = "active-tab", a = "active-pane", b = "aria-selected", e = "aria-hidden";
	AJS.tabs = {
		setup : function() {
		...
		change : function(g, h) {
			...
			f.addClass(a).attr(e, "false").siblings(".tabs-pane")
					.removeClass(a).attr(e, "true");
			g.parent("li.menu-item").addClass(d).siblings(".menu-item")
					.removeClass(d);
...
(function(a, e) {
	...
	a.whenIType = function(q) {
		...
		function k(t) {
			var v = e(t), x = v.attr("title") || "", w = p.slice();
		...
		function f(t, s, r) {
			if (r) {
				s += " ("
						+ AJS.I18n.getText("aui.keyboard.shortcut.type.x", t
								.shift())
			} else {
				s = s.replace(/\)$/, "");
			...
			e.each(t, function() {
				s += " "
						+ AJS.I18n
								.getText("aui.keyboard.shortcut.then.x", this)
			});
			s += ")";
			return s
		}
	...
	a.whenIType.makeShortcut = function(n) {
		...
		function f(o) {
			return function(p, r) {
				...
				i
						.add(function() {
							var w = e(p), v = w.filter("." + q), u = v.length === 0 ? undefined
									: {
										transition : true
									};
							if (t) {
		...
		return {
			...
			unbind : function() {
				e(document).unbind("keydown keypress", g).unbind(
						"keydown keypress keyup", h);
								...
								if (j === "execute" || j === "evaluate") {
									...
									if (/^\[[^\]\[]*,[^\]\[]*\]$/.test(o)) {
										try {
											...
											c("When using a parameter array, array must be in strict JSON format: "
													+ o)
										}
										if (!e.isArray(n)) {
											c("Badly formatted shortcut parameter. String or JSON Array of parameters required: "
													+ o)
										}
									} else {
										n = [ o ]
	...
	AJS.responsiveheader.setup = function() {
		...
		function b(j, o) {
			...
			function d(x) {
				...
				w
						.append(aui.dropdown2
								.contents({
									id : "aui-responsive-header-dropdown-content-"
											+ x,
									extraClasses : "aui-style-default",
									content : aui.dropdown2
											.section({
												content : "<ul id='aui-responsive-header-dropdown-list-"
														+ x + "'></ul>"
											})
								}));
				if (s == 0) {
					...
					w.insertBefore(n(".aui-nav > li > .aui-button").first()
							.parent())
				}
				l = w;
				p = l.outerWidth(true)
			...
			function g(w, B, D) {
				...
				var y = n(".aui-nav > li > a:not(.aui-button):not(#aui-responsive-header-dropdown-trigger-"
						+ D + ")").length;
				while (w > B) {
					A = u[B];
					if (A && A.itemElement) {
						...
						x.children("a").removeClass(
								"aui-dropdown2-sub-trigger active");
						B = B + 1;
						y = y + 1
					}
				}
